...
if (isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
    ...
    // Fetch recipe details and ensure it belongs to the logged-in user
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($title_err) && empty($description_err) && empty($category_id_err) && empty($prep_time_err) && empty($cook_time_err) && empty($servings_err) && empty($recipe_image_err) && empty($ingredients_err) && empty($instructions_err)) {
        ...
        $sql = "UPDATE recipes SET title=?, description=?, category_id=?, health_condition_id=?, prep_time=?, cook_time=?, servings=?, recipe_image=?, ingredients=?, instructions=? WHERE id=? AND user_id = ?";
        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssiissssssii", $title, $description, $category_id, $health_condition_id, $prep_time, $cook_time, $servings, $recipe_image, $ingredients, $instructions, $id, $user_id);
            if (mysqli_stmt_execute($stmt)) {
