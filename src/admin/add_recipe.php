...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["recipe_image"]) && $_FILES["recipe_image"]["error"] == 0) {
        ...
        $imageFileType = strtolower(pathinfo(basename($_FILES["recipe_image"]["name"]),PATHINFO_EXTENSION));
    ...
    if (empty($title_err) && empty($description_err) && empty($category_id_err) && empty($prep_time_err) && empty($cook_time_err) && empty($servings_err) && empty($recipe_image_err) && empty($ingredients_err) && empty($instructions_err)) {
        ...
        $sql = "INSERT INTO recipes (title, description, category_id, health_condition_id, prep_time, cook_time, servings, recipe_image, ingredients, instructions, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssiissssssi", $title, $description, $category_id, $health_condition_id, $prep_time, $cook_time, $servings, $recipe_image, $ingredients, $instructions, $user_id);
...
<main>
    ...
    <form action="add_recipe.php" method="post" enctype="multipart/form-data">
        ...
        <input type="text" id="prep_time" name="prep_time" required value="<?php echo isset($prep_time) ? $prep_time : ''; ?>">
        <span><?php echo $prep_time_err; ?></span>

        <label for="cook_time">Cooking Time:</label>
        <input type="text" id="cook_time" name="cook_time" required value="<?php echo isset($cook_time) ? $cook_time : ''; ?>">
        <span><?php echo $cook_time_err; ?></span>

        <label for="servings">Servings:</label>
        <input type="number" id="servings" name="servings" required value="<?php echo isset($servings) ? $servings : ''; ?>">
